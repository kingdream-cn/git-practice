...
(function() {
  ...
  function completeCommand() {
    ...
    var HELPERS = ['touch', 'echo', 'cat', 'clear', 'help', 'reset-tutorial'];
    var gitSub = Object.keys(GT.commands).filter(function(c) {
      return HELPERS.indexOf(c) === -1;
    });

    var candidates = [];
    if (parts.length === 1) {
      var top = ['git'].concat(HELPERS);
      for (var i = 0; i < top.length; i++) {
        if (top[i].indexOf(last) === 0) candidates.push(top[i]);
      }
    } else if (parts.length === 2 && parts[0] === 'git') {
      for (var j = 0; j < gitSub.length; j++) {
        if (gitSub[j].indexOf(last) === 0) candidates.push(gitSub[j]);
      }
    }
